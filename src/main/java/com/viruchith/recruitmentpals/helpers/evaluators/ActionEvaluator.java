...
public class ActionEvaluator{
	...
	public boolean isUserAuthorized(String... authorizedUsers){
		...
		if(!set.contains(authenticationHelper.getUserType())) {
			errorResponse = new StandardResponse(false,String.format("Only %S users are authorized to perform this action !", Arrays.toString(authorizedUsers)));
			return false;
		}
	...
	public boolean canAdminOrCoordinatorPerformCoordinatorAction(long coordinatorId){
		...
		if(!placementCoordinatorOptional.isPresent()) {
			errorResponse = new StandardResponse(false, String.format("The %S user your'e trying to perform this action on, does not exist !",UserTypes.COORDINATOR));
		}
			...
			if(coordinator.getId()==coordinatorId){
				...
				errorResponse = new StandardResponse(false, String.format("You are not authorized to perform this action on another %S !",authenticationHelper.getUserType()));
			}
		}else {
			errorResponse = new StandardResponse(false, String.format("%S is not authorized to perform this action !",authenticationHelper.getUserType()));
		}

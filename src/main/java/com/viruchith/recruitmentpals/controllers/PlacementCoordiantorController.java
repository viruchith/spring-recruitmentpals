...
public class PlacementCoordiantorController {
	...
	public ResponseEntity<StandardResponse> create(@RequestBody @Valid PlacementCoordinator placementCoordinator) {
		...
		if (ActionEvaluator.isAdmin(authenticationHelper)) {
			PlacementCoordinator coordinator = placementCoordiantorService
					.findFirstByEmail(placementCoordinator.getEmail());

			if (coordinator != null) {
				return ResponseEntity.ok(new StandardResponse(false, String.format(StandardMessages.USER_ALREADY_EXISTS,
						UserTypes.COORDINATOR, "username", placementCoordinator.getEmail())));
			}

			StandardResponse response = new StandardResponse(false,
					String.format(StandardMessages.USER_CREATED_SUCCESSFULLY, UserTypes.COORDINATOR));
			response.addData("coordinator",
					placementCoordiantorService.createPlacementCoordinator(placementCoordinator));
	...
	public ResponseEntity<StandardResponse> fetchAllPlacementCoordinators() {
		authenticationHelper.setAuthentication(SecurityContextHolder.getContext());
		if (ActionEvaluator.isAdmin(authenticationHelper)) {
			List<NoPasswordPlacementCoordinator> placementCoordinators = placementCoordiantorService
					.findAllNoPassword();
			StandardResponse response = new StandardResponse(true,
					"All " + UserTypes.COORDINATOR + " users fetched successfully !");
	...
	public ResponseEntity<StandardResponse> fetchPlacementCoordinatorById(@PathVariable long coordinatorId) {
		authenticationHelper.setAuthentication(SecurityContextHolder.getContext());
		if (ActionEvaluator.isAdmin(authenticationHelper)) {
			NoPasswordPlacementCoordinator placementCoordinator = placementCoordiantorService
					.findFirstByIdNoPassword(coordinatorId);
			StandardResponse response = new StandardResponse(true,
					"All " + UserTypes.COORDINATOR + " users fetched successfully !");
	...
	public ResponseEntity<JwtAuthenticationResponse> login(
		...
		try {
			authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(
					jwtAuthenticationRequest.getUsername() + "|" + UserTypes.COORDINATOR,
		...
		UserDetails userDetails = appUserDetailsService
				.loadUserByTypeAndUsername(jwtAuthenticationRequest.getUsername(), UserTypes.COORDINATOR);

		PlacementCoordinator placementCoordinator = placementCoordiantorService
				.findFirstByEmail(jwtAuthenticationRequest.getUsername());
	...
	public ResponseEntity<StandardResponse> changePassword(@PathVariable long coordinatorId,
		...
		if (actionEvaluator.isUserAuthorized(UserTypes.ADMIN, UserTypes.COORDINATOR)) {
			if (actionEvaluator.canAdminOrCoordinatorPerformCoordinatorAction(coordinatorId)) {
				...
				if (placementCoordinatorOptional.isPresent()) {
					...
					if (ActionEvaluator.isCoordinator(authenticationHelper) && !placementCoordiantorService
							.validatePassword(placementCoordinator, passwordChangeRequest.getPassword())) {
						return ResponseEntity.ok(new StandardResponse(false, "Incorrect Current Password !"));
					} else {
						String encoded = placementCoordiantorService
								.encodePassword(passwordChangeRequest.getNewPassword());
		...
		if (ActionEvaluator.isAdmin(authenticationHelper)) {
			...
			if (!optional.isPresent()) {
				return ResponseEntity.ok(new StandardResponse(false,
						String.format("%S user with ID \"%d\" does not exist !", UserTypes.COORDINATOR, coordintorId)));
			} else {
				placementCoordiantorService.deletePlacementCoordinator(optional.get());
				return ResponseEntity.ok(new StandardResponse(true, String.format(
						"%S user with ID \"%d\" was deleted successfully !", UserTypes.COORDINATOR, coordintorId)));
			}
		}
	...
	public ResponseEntity<?> hello() {
		...
		PlacementCoordinator placementCoordinator = placementCoordiantorService
				.findFirstByEmail(authenticationHelper.getUsername());
